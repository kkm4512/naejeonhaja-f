...
const emit = defineEmits<{
  "update:lolPlayerResultHistorySearchResults": [Page<LolPlayerResultHistoryResponseSimpleDto>];
...
const emitSearchResults = debounce(async (query: string, page: number) => {
  ...
  const response = await uFetch<null, ApiResponse<Page<LolPlayerResultHistoryResponseSimpleDto>>>(null,`/game/lol/${rawDomain}/detailSearch?playerResultHistoryTitle=${query}&page=${page}`,"GET",true);
    emit("update:lolPlayerResultHistorySearchResults", response.data); // 부모에게 검색 결과 전달
    },300);
...
<template>
    <div class="flex justify-end items-center p-4">
      ...
      <input
        ...
        class="w-96 p-2 border border-gray-300 rounded shadow focus:outline-none focus:ring-2 focus:ring-blue-500"
      />
    </div>
  </template>
