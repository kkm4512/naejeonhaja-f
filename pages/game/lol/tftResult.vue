...
import type { TftRequestPlayer } from '~/types/req/reqLolDto';
...
const recomposeTeam = async () => {
  try {
    const reqPlayers = JSON.parse(localStorage.getItem('tftRequestPlayer') || '[]'); // 로컬스토리지에서 reqPlayers 가져오기
    if (!reqPlayers.length) {
      alert('팀 구성 데이터가 존재하지 않습니다.');
      return;
    }

    const response: ApiResponse<TftResponseDto> = await lolFetch<TftRequestPlayer,ApiResponse<TftResponseDto>>(reqPlayers, "tft");
    lolStore.setTftResponseDto(response); // 결과 업데이트
  } catch (error) {
  ...
  <div class="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
    ...
    <div class="flex justify-center gap-10 mb-6 w-full max-w-4xl text-center">
      <div class="flex flex-col items-center">
        ...
        <span class="text-lg font-bold text-blue-800">
          Total MMR: {{ calculateTotalMMR(lolStore.tftResponseDto?.data.teamA || []) }}
        </span>
      </div>
      ...
      <div class="flex flex-col items-center">
        ...
        <span class="text-lg font-bold text-red-800">
          Total MMR: {{ calculateTotalMMR(lolStore.tftResponseDto?.data.teamB || []) }}
        </span>
      </div>
    </div>
    ...
    <div class="flex gap-10 w-full max-w-4xl">
      ...
      <div class="w-1/2 bg-blue-50 p-4 rounded-lg shadow-md">
        ...
        <ul class="space-y-2">
          <li
            v-for="(player, index) in lolStore.tftResponseDto?.data.teamA"
            :key="index"
      ...
      <div class="w-1/2 bg-red-50 p-4 rounded-lg shadow-md">
        ...
        <ul class="space-y-2">
          <li
            v-for="(player, index) in lolStore.tftResponseDto?.data.teamB"
            :key="index"
            :class="`flex items-center gap-4 p-2 rounded-lg border-4 ${getTierGroupClass(player.tier)}`"
          >
